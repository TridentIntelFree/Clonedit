...
$('btnSave').addEventListener('click',()=>{
  ...
  const doc={ fmt:'mvx880-project', build:BUILD, name:$('projName').value,
    ...
    trax:S.trax, inst:S.inst, scOn:S.scOn, scTrig:S.scTrig, scDepth:S.scDepth, scRel:S.scRel,
...
function undoSnap(){
  return { name:$('projName').value,
    ...
    chain:S.chain, chainOn:S.chainOn, chainPos:S.chainPos,
    pattern:S.pattern, bank:S.bank, editPad:S.editPad, seqPad:S.seqPad,
    trax:S.trax, inst:S.inst, scOn:S.scOn, scTrig:S.scTrig, scDepth:S.scDepth, scRel:S.scRel, autoTarget:S.autoTarget,
    song:S.song, songOn:S.songOn, songLoop:S.songLoop, morph:S.morph,
...
const FEEDBACK_ENDPOINT='';   // ← paste your deployed Worker URL here to turn on the online 100-message inbox; empty = share/copy only
// You can also set it without editing this file:  localStorage.setItem('jbh_fb_endpoint','https://…')
function feedbackEndpoint(){ try{ return (localStorage.getItem('jbh_fb_endpoint')||'').trim()||FEEDBACK_ENDPOINT; }catch(e){ return FEEDBACK_ENDPOINT; } }
let lastFbT=0;
function encodeFeedback(obj){
  const j=JSON.stringify(obj);
  try{ return 'jbhfb1:'+btoa(unescape(encodeURIComponent(j))); }catch(e){ return 'jbhfb1:'+btoa(j); }
}
async function fbShareFallback(blob,cat,hasDiag){
  const shareText='JBH-88 beta report ['+cat.toUpperCase()+'] — please forward to the developer:\n\n'+blob;
  if(navigator.share){
    try{ await navigator.share({title:'JBH-88 beta feedback',text:shareText}); $('fbMsg').textContent='✓ Report shared — thank you!'+(hasDiag?' (version + diagnostics included.)':''); $('fbText').value=''; return; }
    catch(e){ if(e&&e.name==='AbortError'){ $('fbMsg').textContent='Share cancelled — tap SEND again to retry.'; lastFbT=0; return; } }
  }
  try{ await navigator.clipboard.writeText(shareText); $('fbMsg').textContent='✓ Report copied — paste it to the developer (message/email). Thank you!'; $('fbText').value=''; }
  catch(e){ $('fbText').value=shareText; $('fbMsg').textContent='Report ready in the box above — copy it and send to the developer.'; }
}
$('fbSubmit').addEventListener('click',async ()=>{
  const msg=$('fbText').value.trim();
  if(msg.length<4){ $('fbMsg').textContent='Add a few words about what happened first.'; return; }
  const now=Date.now();
  if(now-lastFbT<15000){ $('fbMsg').textContent='One report every 15 seconds, please (keeps things tidy).'; return; }
  const cat=$('fbCat').value, hasDiag=$('fbDiag').checked;
  const report={ f:'jbhfb1', v:BUILD, t:new Date().toISOString(), cat, msg:msg.slice(0,1400) };
  if(hasDiag){ try{ report.diag=diagDump('feedback'); }catch(e){} }
  const blob=encodeFeedback(report);
  lastFbT=now;
  // ONLINE inbox path (capped at 100 server-side); fall back to share on any failure
  ...
  if(endpoint && navigator.onLine){
    ...
    try{
      ...
      const res=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({blob}),signal:ctrl?ctrl.signal:undefined});
      if(to) clearTimeout(to);
      if(res.ok){ $('fbMsg').textContent='✓ Sent — thank you! Your report reached the developer’s inbox.'; $('fbText').value=''; return; }
      if(res.status===429){ $('fbMsg').textContent='Inbox is rate-limited right now — please try again in a bit.'; lastFbT=0; return; }
      // other status → fall through to share so nothing is lost
    }catch(e){ /* offline/blocked/timeout → fall through to share */ }
  }
  await fbShareFallback(blob,cat,hasDiag);
});
