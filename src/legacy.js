...
/* ================================================================
   ...
   - R120: THE OCTAVE ONLY WENT DOWN, AND NOTES GOT A CIRCLE.
     Reported: in the sequencer's NOTES lane you could go down an octave but
     never back up. Both buttons and both handlers existed — the row was simply
     too wide. On a 375px screen SCALE and the OCT stepper overflowed and the
     PLUS wrapped to the far left of the line below, where it no longer read as
     belonging to anything. On a 430px screen it fitted, which is exactly why it
     looked fine here and broken on a phone.
     A −/value/+ stepper is one control and must never be split by a wrap, so it
     is an unbreakable unit now and the select beside it gives up the width
     instead. Checked at four widths: same line, adjacent, on screen, clickable.
     And NOTES can be a circle, matching the sequencer above it, which has had
     GRID and CIRCLE since R33. Rings are the scale notes with the root
     outermost, segments are steps, and the playhead sweeps it while the pattern
     runs — a melody reads as a shape rather than a row of boxes. It reuses the
     grid's own toggleNote, so both views write identical pitch locks and cannot
   ...
   - R124: THE RESTORE OFFER WAS STANDING ITSELF DOWN. Reported: no restore was
     offered, but the session was in REWIND. Both halves were the same cause.
     Opening the app schedules an autosave of its own — boot calls setBpm, which
     calls dirty() — so one fires about 1.5s after launch with nobody having
     touched anything. Before R123 that silently overwrote the offered session,
     which is the bug that lost the work in the first place. R123 filed it into
     REWIND instead and took the bar off screen, so the loss stopped but the
     offer now vanished on its own, every single boot, before it could be read.
     An autosave with a pending offer and no audio in the live session has
     nothing worth writing, so it now writes nothing: the offer stands until you
     actually make something. Verified — open, wait five seconds, the bar is
     still there, the vault is untouched, and RESTORE still loads it. Create
     real audio and the offer stands down with the old session filed into
     REWIND, exactly as R123 intended.
     Worth noting what R123 got right: the reported session came back. The
     safety net worked while the mechanism above it was still wrong.
   - R125: A DELETED STEP THAT KEPT COMING BACK. Reported: remove a sound from
     the sequence and it reappears — same pad, same spot. The exported session
     had it exactly: CHAIN on, running PTN 1 → PTN 2, and pad 1 carrying hits in
     BOTH patterns.
     The editor and the transport share one "current pattern". While a chain or
     song runs it calls selectPattern every bar, so the grid moves under you and
     the step you tap next lands on whichever pattern arrived — not the one you
     were looking at when you decided to tap. Delete a hit, let the arrangement
     come round to the pattern that still has it, and the app looks like it
     ignored you.
     What the arrangement does is unchanged. It just cannot happen behind your
     back now: while a chain or song is driving, the SEQ view says so, names the
     pattern showing and the running order, and offers HOLD ONE PATTERN to turn
     the arrangement off so the grid stays put. Editing a step while it runs
     also reports which pattern took the edit, and that the pad may have hits in
     the others.
     The real cure is separating "the pattern playing" from "the pattern being
     edited" — a bigger change than this warranted today, and worth doing.
   - R126: WHAT THE SEQUENCER SHOWS IS WHAT IT PLAYS AND WHAT IT SAVES.
     Stated as a rule, and the engine broke half of it. Shortening a track or a
     pattern hid the steps past the new end without removing them: they stopped
     sounding, so playback and the grid still agreed — but they stayed in the
     pattern, went into every save, and came back the instant the length grew
     again. Delete a hit, shorten, lengthen, and the hit you deleted is playing
     again in its original place, which is the app overruling you.
     Trimming is destructive now, on purpose. A row eight steps long contains
     eight steps. LEN and PTN LEN both clear what is past the end — steps, their
     pitch locks, and the silencer lane with them — and say how many they threw
     away. Loading an older project trims it on the way in, so files already
     carrying ghosts are cleaned rather than preserved.
     Proved rather than asserted. The suite checks the grid, the pattern in
     memory and the saved file list identical steps; that shortening removes
     rather than hides; that lengthening resurrects nothing; that every track is
     trimmed and not just the visible one; and that an old project loads clean.
     The audible half is settled by rendering the same pattern twice and
   ...
   - R131: A MIX SLAMMED INTO THE LIMITER NOW SAYS SO. Reported: "a strange
     glitchy sound happens upon playback, it doesn't seem intentional", with a
     DIAG showing pad A01 at 200% with a 97% DELAY send, A02 at 133% and the
     master at 107%.
     Rebuilding that session and measuring it: 2.57 arrives at a limiter set to
     a -1dB ceiling. The limiter pulls the mix down 8.5dB at worst and 5.7dB on
     average, and 29% of the sound ends up in the soft-clipper's saturation.
     Sane gains on the same pattern: 0.79 in, 0.7dB of reduction, 5%. The
     squashing and the saturation ARE the glitch.
     Nothing clips — the soft-clip stage makes that impossible, output peaks at
     0.932 — and that is exactly the trap. Every meter says nothing is clipping
     while the sound falls apart, so there is no thread to pull on. There was a
     hint for it, and it was scoped to the OUT tab: a tab you have no reason to
     be on while you are hearing the problem. It now says so wherever you are,
     after 3dB of reduction sustained for two and a half seconds, naming the
     loudest pad and pointing at AUTO. At most once a minute.
     (My first measurement said the output was clipping at 1.139. It was taken
     at the limiter, which is upstream of the makeup-cancel and the soft-clip —
     the two nodes that exist to undo precisely that. Measured at the end of the
     chain, nothing is over.)
     DIAG gained the two things that would have answered this from the paste.
     The delay line now shows the bpm and division behind the time, and flags
   ...
   - R135: THE TAKE — a bounce plays the same performance every time.
     I had said renders differed by about -75dB from "an untraced random
     element". Both halves of that were wrong, and measuring properly took the
     answer apart into three pieces.
     One: the demo project ships with HUMANIZE at 0.3 and thirteen PROBABILITY
     locks. Those are supposed to vary — but the consequence was that bouncing
     the same project twice gave two different PERFORMANCES, -11dB RMS apart,
     with nothing saying so. Export, listen, like it, export again to fix the
     filename, get a different record. The render now draws from a seeded
     generator: the seed is part of the project, rides in the autosave, the JSON
     export and every undo step, and NEW TAKE rolls a fresh one. Same project,
     same take, today and next year. Measured: seeding buys 23dB of agreement.
     Two: with humanize and probability off, Math.random is called ZERO times
     during a render, so the remainder was never random. It starts at about
     -150dB — one float32 bit — and the delay feedback and master compressor
     amplify it. Bisecting by voice count found the threshold exactly: one to
     four sources render bit-identically, six or more never do, whether they
     overlap or not. That is the browser summing more than a handful of nodes in
     an order it does not promise to repeat; float addition is not associative.
     It is not ours and no amount of seeding touches it. Typically -74dB below
     the mix, worst case around -23dB in the densest passages.
     Three: the WAV writer's TPDF dither, about -93dB, deliberate and documented.
     And chasing this turned up something else. Boot said READY · load a sample
     into a pad BEFORE offerRestore() had decided what to do — and for a new
     user what it does is spend a couple of seconds building the demo song and
     then replace every pad and pattern. So the app claimed a state it was not
     in, invited work, and threw that work away when the song arrived. It says
     STARTING now, and whichever branch runs writes READY only when it is true.
     Found because a test that writes steps during that window had its steps
     silently overwritten — which is exactly what would happen to a person.
   - R136: "PRESS PLAY TO READ THE OUTPUT" — SAID TO SOMEONE WHO WAS PLAYING.
     Reported with a screenshot: transport lit, spectrum moving, PEAK -21.8 and
     RMS -32.1 both reading, LOUDNESS a dash — and the panel underneath saying
     press play. The whole advice line was chosen from the loudness reading
     alone, so one meter returning nothing was taken as evidence about the
     transport. It reads the transport now, and when loudness cannot read while
     playing it says which of the two things that means.
     The loudness tap itself was rebuildable-once: the guard was `if(kAn) return`,
     but the tap holds a connection to ONE graph's limiter. Replace the graph and
     it keeps analysing a node nothing feeds, silently, forever — while PEAK and
     RMS carry on because they look their analyser up fresh every frame. It
     remembers which graph it belongs to now and re-binds. DIAG reports whether
     the tap exists, whether it is bound to the current graph, and the TONE
     settings.
     (I could not reproduce the dash in Chromium — loudness reads there. So the
     honest fix was the message and the guard, plus enough in DIAG to identify
     it next time. Same shape as the mic: a WebKit-only silence that every check
     passes through.)
     Also reported: AUTO "bottomed out a lot of stuff it didn't seem to need to,
     I was able to bring it way back up and still not clip." Measured, AUTO is
     right. Its worst-case pass sits 0.67dB above the take you actually get and
     0.12dB BELOW the loudest of ten random takes, so the safety margin is real
     and small. What was missing is why it moved so far: TONE was at +11.5 /
     +4 / +12dB, and TONE sits before the trim, so AUTO had to take that back
     out again. It says so now when the boost is over 3dB.
     And "still not clipping" is not evidence of headroom — nothing in this app
     can clip, the soft-clip stage forbids it. That is the R131 trap, and the
     limiter warning is what answers it.
   - R137: THE LOUDNESS READOUT WAS NOT MEASURING LOUDNESS.
     Asked to fix it properly, and to find anything else fake.
     LOUDNESS was a K-weighted RMS of ONE analyser frame: 46ms of a mono
     downmix, ungated, through approximate filters — sitting in a row with PEAK
     and RMS, which are true. Against a real BS.1770 implementation on the same
     bounce it read -6.4 where the answer was -11.1. 4.6dB optimistic: aim a
     master at -14 with that and you deliver -18.6.
     Replaced with the actual standard. K-weighting is the specified pair, a
     +3.999dB shelf at 1681.97Hz and an RLB highpass at 38.135Hz, derived at the
     context's own rate rather than hard-coded for 48k. The live meter reads
     momentary (400ms) and short-term (3s) from BOTH channels summed as mean
     squares, and is labelled LUFS M / S because that is what it is. BOUNCE
     computes the integrated, twice-gated figure from the finished buffer, plus
     true peak, and says how it sits against the -14 LUFS streaming target.
     True peak is a polyphase 4x windowed-sinc interpolation, and it only
     interpolates around samples within 3dB of the peak — exact where it can
     matter, 14x faster, agreeing with brute force to 0.0000dB.
     Everything is validated against signals with known answers rather than
     against itself: a sine at sr/4 sampled on its zero crossings (samples
     -3.01dB, true peak 0.00dB), DC (true peak must equal the level), and tones
     that must track level exactly and sum to 3.01dB between one channel and
     two. Three of my own attempts failed that validation first — two broken
     interpolators, and a reference figure I had simply got wrong.
     The audit also CLEARED things, which is worth recording. The WAV really is
     44.1k / 16-bit / stereo, header and payload. AUTO really is accurate. And
     true peak is currently a non-issue for this app — 0.00dB above sample peak
     — because the soft-clip stage keeps everything about a dB below full scale.
     No inventing a problem that is not there.
   ...
   - R140: THE OFFLINE BADGE COULD NOT BE READ ON AN UPRIGHT PHONE.
     ...
     ship.
   - R141: THE CHECKS LIVE IN THE REPO NOW, AND MIDI IS FUNCTION-CHECKED.
     Everything verifying this app was a script in a temp directory on one
     machine — 289 of them — which is the same as not having it. `npm test`
     covered src/pure/: 53 tests over ~330 lines, about 3% of the code.
     tests/browser/ now holds six suites run by `npm run test:browser`, with a
     shared harness (one answer to "which Chromium", shared with the build
     gate so the two can never disagree about what they ran against) and a
     GitHub Actions workflow that runs unit tests, the build, and the browser
     suites on every push. CI also fails if index.html has drifted from what
     src/ builds — the build is byte-reproducible, so that check is exact.
     MIDI is the reason this happened now: no controller to hand, and a whole
     section of the app resting on it. A fake MIDIAccess is installed before
     the app loads, so MIDI ON runs the real midiInit — real enumeration, real
     select population, real rebindInputs, real bindMidiOutput. Input is
     delivered through the handler the app itself installed on the fake device
     rather than by reaching past it into onMidi, so the wiring is under test
     too; output is captured at the device's send() with the timestamps the app
     computed. That last part is what makes the clock checkable rather than
     assumed: 24 PPQN at 120 BPM measured at 20.833ms between ticks, and
     41.667ms when the tempo is halved. Notes, CCs, CC LEARN, program change,
     all four velocity curves, channel filtering, chromatic transposition,
     external clock in (tempo derived from the incoming ticks, 16 steps from 96
     ticks), BLE running-status parsing, and all-notes-off — 40 checks.
     What it cannot do is say a particular controller works: nothing here
     touches USB driver behaviour, real jitter, or what byte a given pad sends.
     It establishes that if the bytes arrive the app does the right thing, and
     that what it sends is correct and correctly timed. The rest needs hardware.
     Writing the suites found no app bugs and four of my own, which is the
     honest result to record. Three were mine outright — an "unmapped" MIDI note
     that all 64 pads already own, a tolerance tighter than the test's own
     rounding. The fourth was a misunderstanding worth keeping: I assumed
     shortening a pattern HID the steps past the end and restored them on
     lengthening. It does not. trimTrack removes them and says "N hits past the
   ...
   - R147: ONE NUMBER — A PAD WITH ITS OWN BPM. "Can we have an easier to
     ...
     accepted a whole number of hits per bar, which called 3.5 — round every two
     bars — drifting, when a listener plainly hears it lock.
     A third, found by playing an old project back rather than by any assertion:
     the scheduler re-anchored every BAR, but a rate of 3.5 hits per bar only
     returns to a downbeat every SECOND bar. Anchoring per bar forced a hit onto
     every downbeat, so a converted 7-over-2 came out as 0 · 0.686 · 1.371 ·
   ...
   - R155: THE DRAIN WAS ON THE WRONG HOOK. "My playback is messed up now, it has
     a distortion that's not from effects, didn't appear before." Mine, from
     R154, and the word "now" was the whole clue.
     flushTails was hung off panicVoices — but panicVoices is not STOP. CLR PTN
     calls it, and so does loading a project, so clearing a pattern mid-song
     ducked the reverb and delay sends to zero and let them swell back in
     underneath the music two seconds later. And pressing PLAY inside that window
     started the song with no effects at all, arriving late. Both are the same
     mistake the tail itself was: something audible happening on its own
     schedule rather than on yours.
     The drain belongs to stopSeq, which is what "STOP means silence" always
     meant, and startSeq now cancels a drain outright rather than waiting for its
     timer. Verified: STOP ducks, PLAY restores in the same frame, the cancelled
     timer cannot fire under the music, and CLR PTN mid-song leaves the effects
     exactly where they were.
     Checked the rest of R153-R154 the same way rather than assuming: the demo
   ...
   - R158: "AMBER SIGNAL" LEARNS THE NEW TRICKS. Asked to update the factory song
     to use POLY and NOTES, and to be creative about where the sounds come from.
     A feature the demo does not touch is a feature nobody meets.
     POLY in the two places a sixteen-step grid genuinely cannot go. Both
     choruses: the closed hat keeps ordinary time except on steps 7 and 14,
     which open into three hits inside one beat — 276 against a project at 92.
     The breakdown: congas three across the WHOLE bar at 69, three quarters of
     the tempo, against a kick squarely in four.
     PLAIN HIT earned its place immediately. With the beat-long figure firing
     from all six lit hats it overlapped four deep and measured as hits 54ms
     apart across the bar — a machine gun, not a triplet. Marking the
     timekeeping hats plain leaves two flourishes and it plays.
     NOTES: the warm pad held single notes, wasting the one instrument in the
     kit that can carry a triad without turning to mud. Am/F and C/G now.
     SOUNDS THAT ARE NOT IN THE PRESET LIST. Two pads are built rather than
     chosen — two renders summed with one arriving late, which is what a player
     does with two layers on a pad. A11 is a pluck with a bell 12ms behind it,
     close enough to read as the string's own ring; A14 is cello + glass played
     in GRAIN mode, so the pad sprays fragments of a pair that exists nowhere
     in the list. A15 is the riser reversed — an intake, not a push. A12 drops
     an octave in the breakdown and holds its length because KEEP TIME landed
   ...
   - R161: WHAT SURVIVES BEING SAVED. Three findings from a QA pass on R160,
     all reproduced before being fixed. Two of them were worse than reported.
     A LOAD COULD FAIL HALFWAY. docAccept's own comment states the invariant:
     every route in must be refused whole or applied whole. Two fields broke
     it. masterVol and delayFb were assigned raw where every neighbour carried
     a default, and both are written straight into AudioParams, which throw on
     a non-finite value — so a document missing either failed a third of the
     way down, AFTER pads, patterns and tape lanes had been replaced, and
     printed LOAD FAILED over a session that had in fact been half-overwritten.
     They carry defaults now, and docAccept checks every number that reaches an
     AudioParam before anything is applied, so the invariant is enforced at the
     gate rather than depending on each future field remembering its own
     default.
     SAVING CLIPPED ANYTHING OVER 0dBFS. Reported against the autosave vault;
     it was every persistence path, because f32ToB64 — the .json project file
     and the library — is also Int16 despite its name. The TRAX tap sits
     upstream of the limiter deliberately, so takes above 1.0 are normal and
     the float path carries them; 32-bit export was measured carrying 1.9
     intact. Saving flat-topped every one, permanently. Measured: a source
     peaking 1.9 came back at 1.00003 with 704 samples pinned flat.
     AND IT WASTED THE QUIET END. Undithered Int16 error is fixed in absolute
     terms, so quiet material loses most: a channel at -48dBFS kept 8.0
     effective bits.
     One scale factor per channel fixes both. The channel is normalised on the
     way in and multiplied back on the way out: nothing clips at any level, and
     every channel gets the whole grid regardless of level — the same -48dBFS
     channel now keeps 16.0 bits. It costs one float per channel and not one
     byte of sample data. Absent pk means 1, exactly the old behaviour, so
     older projects still open; DOC_V goes to 3 so an older build refuses these
     files rather than playing a normalised take at the wrong level.
     THE GUARD NEARLY BECAME THE BUG. DOC_NUMS is a hand-written list of field
     names, and naming a field that is not numeric makes docAccept refuse EVERY
     real project — far worse than what it prevents. It happened twice: morph
     is an object, vcurve is the string 'linear'. The first slipped past
     because the test only asserted the load did not THROW, which a blanket
     refusal also satisfies. The test asserts the load RETURNS TRUE now, and a
     separate check walks DOC_NUMS against a real document so the list cannot
     drift from the schema.
   - R162: THE GUARD DID NOT GO DEEP ENOUGH. Four findings from an adversarial
     QA pass on R161. The first is the same invariant, broken one level down.
     PAD NUMBERS BYPASSED THE GATE, AND THE CORRUPTION REACHED DISK. DOC_NUMS
     validates 29 TOP-LEVEL scalars; nothing looked inside pads[]. But
     pads[].gain, .pan, .rev and .dly go to hardSet and then to an AudioParam,
     so a document valid everywhere except pads[0].gain='banana' passed the
     gate and threw at the pad stage — with S.bpm already moved to the poisoned
     document's tempo and S.pads[0].gain now holding the string. Worse than the
     R161 case, because it did not stop at the session: autosave wrote the
     poisoned pad to the vault, and RESTORE brought it back, leaving the state
     and the audio graph disagreeing permanently with nothing to say so.
     No list is written out for it. newPad(0) and newTrack() ARE the schema, so
     any field that is a number in the reference must be a finite number in the
     document if present. That is the direct lesson of DOC_NUMS, which had to
     be corrected twice for naming fields that were not numbers: a check
     derived from the shape cannot drift away from the shape.
     UNDOING TO THE BOTTOM WIPED THE CRASH-RECOVERY VAULT. Undo itself is
     correct and redo recovers everything — but undoing to the bottom leaves
     zero buffers, and dirty() then fires an ordinary autosave, so the vault
     went from sixteen buffers to none. The undo stack lives in memory only, so
     losing the page in that window (a crash, an iOS tab reap) takes redo with
     it while the vault holds an empty session. An empty session no longer
     overwrites a vault that holds audio: the old one is filed into REWIND and
     the slot is left alone, once per distinct vault so nothing churns.
     A HOSTILE SCALE FACTOR. buffers[].pk was read straight from the document
     and pcmRestore only checked pk>0 && isFinite, which 1e30 satisfies —
     producing samples around 9.4e29 in a live buffer. The limiter and soft
     clip absorbed it (0.853 against a 0.843 baseline, no non-finite samples,
     even with reverb and feedback up), so this is hardening, not a fix for
     anything audible. Clamped to 64x full scale.
   ...
   - R165: SAY WHETHER THERE IS ANY AUDIO, AND LET THE REPORT BE COPIED.
     A silence report arrived with a DIAG that looked perfect — context
     running, gates open, pad unmuted, nothing dangling, buffers:1
     loadedPads:1 — because nothing in it said whether that one buffer held
     sound. It did not. DIAG prints a peak per buffer now, with what each is
     loaded on, and SILENT rather than a number when there is nothing there,
     plus a lane line naming the source and any open input. A dump that cannot
     tell "healthy and playing nothing" from "healthy and holding silence"
     sends everyone to look at the graph, which was the one part that was fine.
     The silent-take warning on TRAX already existed — I misread it as missing
     — but it listed all three sources and asked you to check, which came back
     as "the source wasn't clear to me". It knew which source it had used and
     made you work it out anyway. It names the source now, and says what that
     source captures; the success message names it too.
     And a COPY button beside DIAG, asked for directly. A report that is hard
     to send is a report that does not get sent.
   - R166: PREVIEW A TAPE LANE. Asked for. The only way to hear a take was to
     press PLAY and wait for it to come round with the whole arrangement,
     which is no way to answer "did that actually record?" — the exact
     question you have after a take you are unsure of.
     Its own path on purpose: dry, straight to master, ignoring the lane's
     mute, solo, volume and FX, because the point is to hear WHAT WAS
     CAPTURED rather than what the mix is doing with it, and a lane muted at
     zero is the one you most need to audition. It says it is bypassing them
     rather than leaving that a surprise, reports the peak, and says outright
   ...
   - R169: THE OUTPUT CATEGORY BECOMES A CHOICE. Since R111 the app has forced
     navigator.audioSession.type='playback' whenever nothing is recording — on
     essentially every touch, because resumeSession is bound to touchstart.
     That is an override of the browser's own routing judgement, and the spec's
     default is 'auto'. Forcing a category is the unusual move and a plausible
     reason a paired speaker stops being chosen.
     It cannot be tested anywhere here: Chromium does not implement
     navigator.audioSession, so that line is a no-op in every suite. So it is a
     setting rather than a guess — LET THE BROWSER CHOOSE hands the decision
     back, ASK FOR PLAYBACK keeps exactly what the app has always done, and the
     default does not move without evidence. Remembered per device, since it
     describes the hardware rather than the music. What IS tested: the choice
     reaches the API, a live capture still overrides it, the preference returns
     when the capture ends, and DIAG reports both it and whether the API exists
     at all.
     NOTE FOR THE NEXT PERSON READING THIS: "a pad plays through Bluetooth and
     a tape lane does not, in the same session" cannot be a routing fault.
     Both go tail → trackBus → mLo → … → softclip → MediaStreamDestination →
     the hidden audio element. One context, one output, one route. If one is
     heard and the other is not, the other is not producing audio — check the
     buffer peaks line in DIAG, or preview the lane, before looking at routes.
   - R170: CLOSING THE MIC RE-PICKS THE OUTPUT. The report narrowed itself to
     the exact sequence: "Only sessions where I arm mic then turn it back off
     are affected", and "I can restore the session and it plays through bt".
     That is the whole diagnosis. iOS chooses the output route when the audio
     session activates and does NOT revisit it because the category changed
     back — so opening the mic moved sound to the handset, correctly, and
     closing it set the category to playback and left the sound where it was.
     A reload fixed it because a reload builds a new context, and a new context
     gets a new route.
     The falling edge — the last input closing — now rebuilds instead of
     relabelling. Stopped, that is the full context rebuild, which is what a
     reload does and is certain; buffers, pads and takes survive it, verified.
     Rolling, it is the lighter output rebuild, because silently stopping
     someone's playback to fix a routing nicety is not a trade worth making,
     and the message points at RESET OUTPUT if the light one does not take.
     It did not fire at first: the five capture sites set the category by hand,
     so applyAudioRoute never saw an input open and never saw one close. One
     claimRecordRoute() marks the state as well as making the request.
     THE SESSION IS NO LONGER REWRITTEN ON EVERY TOUCH. resumeSession is bound
     to touchstart and ends in applyAudioRoute, so the app was asking iOS to
     configure the audio session under your finger, continuously, almost always
     to the value it already held — and configuring a session is exactly what
     makes iOS re-evaluate a route. Fifty touches now write nothing.
     AND AudioContext.close() IS ASYNCHRONOUS. try/catch around it catches a
     synchronous throw and nothing else, so closing an already-closed context
     escaped as an unhandled rejection — "Cannot close a closed AudioContext",
     surfaced by the new rebuild paths overlapping. Every close goes through
     closeCtx() now.
   - R171: MOVING A PAD EFFECT COST THE AUDIO THREAD. "When I mess with
     settings (up or down) on my pads effects it wrecks them timing wise,
     distorts in a strange way." Both symptoms came from the cost of the move
     rather than from the setting itself, and there were two causes.
     TWELVE KILOBYTES OF GARBAGE PER INPUT EVENT. Every event rebuilt a
     1024-float drive curve AND a 2048-float crush curve and handed both to a
     WaveShaperNode — roughly 720KB a second through a drag, plus a table the
     audio thread has to pick up each time. A garbage collector running under a
     real-time thread is heard as dropouts, and a dropout inside a note is
     heard as distortion. A drive rounded to 1% has a hundred possible curves
     and a crush has sixteen, so they are built once and kept.
     Assignment is skipped when the value has not changed — compared against
     what we last set, not against node.curve, because Chrome's getter hands
     back a COPY and the reference never matches. Measured, not assumed: fifty
     identical calls now reach the node zero times instead of fifty.
     AND THE ANTI-CLICK DIP FOUGHT ITSELF. It ramped the FX gain to silence and
     used a 6ms setTimeout to bring it back — right for one change, wrong for a
     drag, where events arrive faster than the timeout returns and the ramps
     cancel each other. That is an amplitude modulation at the rate of your
     finger, which is the strange distortion, and it could leave the gain
     sitting at 0.0001 with the pad simply gone. One dip per burst now: while
     one is in flight the newest values are recorded and the pending write
     takes them, so the last thing you chose lands and the gain moves exactly
...
/* CURVES ARE CACHED, BECAUSE DRAGGING A SLIDER REBUILT THEM SIXTY TIMES A
   SECOND.

   Reported as pad effects wrecking the timing and distorting strangely while
   the settings were being moved. Both symptoms come from the cost of the move
   rather than from the setting: every input event rebuilt a 1024-float drive
   curve AND a 2048-float crush curve — 12KB of garbage per event, so about
   720KB a second through a drag — and handed both to a WaveShaperNode, which
   makes the audio thread pick up a new table each time. That is a garbage
   collector running under a real-time thread, which is heard as dropouts, and
   dropouts in the middle of a note are heard as distortion.
   A drive amount rounded to 1% has a hundred possible curves and a crush has
   sixteen, so they are built once and kept. Identical values now return the
   identical array, which also lets the caller skip the assignment entirely. */
const driveCurves=new Map(), crushCurves=new Map();
function makeDriveCurve(amt){
  if(amt<=0) return null;                      // null curve = pass-through
  const key=Math.round(clamp(amt,0,1)*100);
  let c=driveCurves.get(key);
  if(c) return c;
  const n=1024, k=1+(key/100)*40, norm=Math.tanh(k);
...
function makeCrushCurve(bits){
  ...
  const key=clamp(bits|0,1,15);
  let c=crushCurves.get(key);
  if(c) return c;
  const n=2048, steps=Math.pow(2,key);
  c=new Float32Array(n);
  for(let i=0;i<n;i++){ const x=i/(n-1)*2-1; c[i]=Math.round(x*steps)/steps; }
  crushCurves.set(key,c);
  return c;
}
...
function applyPadFx(n, p, ctx, live){
  ...
  const abrupt = Math.abs(drv-(n._drv==null?drv:n._drv))>0.15
    ...
    || n.flt.type!==typ;
  const write=()=>{
    /* Only assign when it actually differs. Handing a WaveShaper the same
       table again is not free — the audio thread has to take up a new curve —
       and with the cache, equal values are the same object.
       Compared against what we last set rather than against node.curve:
       Chrome's getter hands back a COPY, so reading it never matches what you
       put in and the skip would never fire. Measured, not assumed. */
    const dc=makeDriveCurve(+p.drv||0), cc=makeCrushCurve(p.crush|0);
    if(n._drvCurve!==dc){ n.drv.curve=dc; n._drvCurve=dc; }
    if(n._crushCurve!==cc){ n.crush.curve=cc; n._crushCurve=cc; }
    n.flt.type=typ;
  ...
  /* ONE DUCK PER BURST.
     ...
     The dip exists so an abrupt curve change does not click. It scheduled a
     ramp to silence and a setTimeout 6ms later to ramp back — fine for one
     change, wrong for a drag, where events arrive faster than the timeout
     returns. Overlapping ducks cancelled each other's ramps: the gain could be
     driven up and down at the rate of the drag, which is an amplitude
     modulation in the audible range and sounds like distortion, and it could
     be left sitting at 0.0001 with the pad simply gone.
     While a dip is in flight the newest values are just recorded; the pending
     write picks them up, so the last thing you chose is what lands and the
     gain moves exactly twice however long you drag. */
  if(n._fxPend){ n._fxWrite=write; return; }
  n._fxPend=true; n._fxWrite=write;
  ...
  setTimeout(()=>{
    try{ (n._fxWrite||write)(); }catch(e){}
    n._fxWrite=null; n._fxPend=false;
...
/* The output cannot clip — the soft-clip stage forbids it — so a mix driven far
   ...
   the sound: distortion and pumping that a listener reasonably calls "glitchy"
   and cannot find a cause for, because every meter says nothing is clipping.
   There WAS a hint for this, on the OUT tab — a tab you have no reason to be on
   while you are hearing it. Say it wherever you are, once a minute at most. */
// -1e9, not 0: performance.now() starts near zero, so a "not in the last
// minute" test against 0 silences the whole first minute of the session.
let limHeavyT=0, limSaidT=-1e9;
function limiterWatch(now){
  if(!LIVE||!LIVE.limiter||!playing) { limHeavyT=0; return; }
  let red=0; try{ red=LIVE.limiter.reduction; }catch(e){ return; }
  if(red>-3){ limHeavyT=0; return; }          // under 3dB is a limiter doing its job
  if(!limHeavyT){ limHeavyT=now; return; }
  if(now-limHeavyT < 2500) return;            // sustained, not one loud transient
  if(now-limSaidT < 60000) return;
  limSaidT=now; limHeavyT=now;
  // name the loudest offender, because "turn something down" is not actionable
  let worst=-1, wg=0;
  for(let i=0;i<NPADS;i++){ const q=S.pads[i];
    if(q.bufId>=0 && !q.mute && q.gain>wg){ wg=q.gain; worst=i; } }
  lcd('The limiter is pulling the mix down '+Math.abs(red).toFixed(1)+'dB — that squashing is what you hear.'
    + (worst>=0 && wg>1.1 ? ' '+padName(worst)+' is at '+Math.round(wg*100)+'%.' : '')
    + ' OUT ▸ AUTO sets the level so nothing has to be squashed.');
}
...
$('btnMicOn').addEventListener('click',()=>{
  if(micOn){ routeNote=''; micDisable(); lcd('MIC OFF.'+routeNote); routeNote=''; }
  else micEnable();
});
...
/* Rebuilt-graph guard: the old test was `kAn` alone, so once the tap existed it
   was never rebuilt — and it holds a connection to ONE graph's limiter. If the
   graph is ever replaced the tap keeps analysing a node nothing feeds any more
   and loudness reads silence forever, while PEAK and RMS carry on working
   because they look their analyser up fresh every frame. Remember which graph
   the tap belongs to, and say why if it could not be built at all. */
let kGraph=null, kErr='', kSplit=null, kChain=[], kBufs=[];
/* The old tap was one analyser on a MONO downmix, read one frame at a time —
   46ms, ungated, through generic filters — and the result was labelled
   LOUDNESS. BS.1770 wants each channel K-weighted separately, summed as mean
   squares, over 400ms (momentary) and 3s (short-term). So: split to two
   channels, the SPECIFIED filters on each (derived at this context's rate, not
   hard-coded for 48k), and a ring of block energies the meter loop fills. */
const K_BLOCK=0.1;                       // 100ms energy blocks
const kRing=[[],[]];                     // per channel, newest last
function buildLoudTap(){
  if(!AC||!LIVE) return;
  if(kAn && kGraph===LIVE) return;
  try{
    const c=k1770Coeffs(AC.sampleRate);
    const mk=co=>{ const f=AC.createIIRFilter(
      [co.b0,co.b1,co.b2],[1,co.a1,co.a2]); return f; };
    kSplit=AC.createChannelSplitter(2);
    kChain=[]; kBufs=[]; kRing[0].length=0; kRing[1].length=0;
    LIVE.limiter.connect(kSplit);
    for(let ch=0; ch<2; ch++){
      const s1=mk(c.shelf), s2=mk(c.hp), an=AC.createAnalyser();
      an.fftSize=4096;
      kSplit.connect(s1, ch); s1.connect(s2); s2.connect(an);
      kChain.push({s1,s2,an});
      kBufs.push(new Float32Array(an.fftSize));
    }
    kAn=kChain[0].an;                    // kept so existing checks still see a tap
    kGraph=LIVE; kErr='';
  }catch(e){ kAn=null; kGraph=null; kChain=[]; kErr=(e&&e.message)||String(e); }
}
/* Called from the meter loop: fold this frame's per-channel mean square into
   the ring, then read momentary (400ms) and short-term (3s) off it. */
function loudRead(){
  if(!kChain.length) return {m:-Infinity,s:-Infinity};
  const sr=AC.sampleRate;
  for(let ch=0; ch<2; ch++){
    const an=kChain[ch].an, buf=kBufs[ch];
    an.getFloatTimeDomainData(buf);
    let ms=0; for(let i=0;i<buf.length;i++) ms+=buf[i]*buf[i];
    ms/=buf.length;
    const r=kRing[ch];
    r.push({t:AC.currentTime, ms});
    while(r.length && AC.currentTime-r[0].t>3.2) r.shift();
  }
  const meanOver=(secs)=>{
    let z=0, any=false;
    for(let ch=0; ch<2; ch++){
      const r=kRing[ch]; let sum=0, n=0;
      for(let i=r.length-1;i>=0;i--){ if(AC.currentTime-r[i].t>secs) break; sum+=r[i].ms; n++; }
      if(n){ z+=sum/n; any=true; }
    }
    return any&&z>0 ? -0.691+10*Math.log10(z) : -Infinity;
  };
  return { m:meanOver(0.4), s:meanOver(3.0) };
}
function outDraw(){
  const cv=$('spectrum');
...
function outMeters(){
  ...
  // BS.1770 momentary (400ms) and short-term (3s), both channels, real filters
  const lr=loudRead();
  const loud=lr.s>-Infinity?lr.s:lr.m;      // the advice line reads short-term
  $('mLufsV').textContent = lr.m===-Infinity ? '—'
    : lr.m.toFixed(1)+(lr.s>-Infinity?' / '+lr.s.toFixed(1):'');
  ...
  /* This used to be the only test, so a silent LOUDNESS reading told anyone who
     WAS playing to press play — while PEAK and RMS sat right beside it showing
     real numbers. One dash is not evidence about the transport. */
  else if(loud===-Infinity) msg = !playing
    ? 'Press PLAY to read the output.'
    : (kAn ? 'Playing, but loudness is reading nothing — check the mixer for a mute or a fader at zero.'
           : 'Loudness is unavailable in this browser'+(kErr?' ('+kErr+')':'')+' — PEAK and RMS above are still true.');
...
$('btnMAuto').addEventListener('click',async ()=>{
  ...
  try{
    ...
    /* Say WHERE the level it removed came from. TONE sits before the trim, so
       boosting the mix 12dB there means AUTO has to take 12dB back out — which
       from the outside looks like AUTO bottoming everything for no reason.
       Naming the cause turns an arbitrary-looking number into arithmetic. */
    const boost=Math.max(S.mEqLo||0,S.mEqMid||0,S.mEqHi||0);
    const why = boost>3
      ? ' TONE is adding up to +'+boost.toFixed(1)+'dB before this point, which is most of what came back off.'
      : '';
    if(want<-24 || want>12)
      ...
      lcd('AUTO — down '+Math.abs(moved).toFixed(1)+'dB. You were '+over.toFixed(1)+'dB into the limiter;'
        +' it has nothing left to do.'+at+why);
    else if(moved>0.05)
...
document.querySelectorAll('#tabs button').forEach(b=>b.addEventListener('click',()=>{
  ...
  if(b.dataset.v==='proj'){ $('bSrc').value=(S.songOn&&S.song.length)?'song':((S.chainOn&&S.chain.length)?'chain':'pat'); drawProjects(); drawRewind(); drawTake(); }
...
function resetAudioOut(){
  ...
  forgetAudioRoute();          // the session was torn down; re-assert rather than assume
  applyAudioRoute();
...
/* WHICH CATEGORY TO ASK FOR WHEN NOTHING IS RECORDING.

   Since R111 the app has forced 'playback' whenever no input is open — on
   essentially every touch, because resumeSession is bound to touchstart. That
   is an override of the browser's own routing judgement, and the Web Audio
   Session spec's default is 'auto', where Safari decides. Forcing a category
   is the unusual choice, and a plausible reason a connected Bluetooth speaker
   stops being picked when it used to be.
   I cannot test that here: Chromium does not implement navigator.audioSession
   at all, so this code path is a no-op in every test I can run, and the only
   evidence available is from a phone with a speaker paired to it. So it is a
   setting rather than a guess — 'auto' hands the decision back to the browser,
   'playback' keeps exactly what the app has been doing. It is remembered per
   device, not per project, because it describes the hardware you are on.
   Default stays 'playback' so nobody's behaviour changes by surprise; it moves
...
function setSessPref(v){
  ...
  forgetAudioRoute();          // a deliberate change must reach the OS even if it matches the last write
  applyAudioRoute();
  lcd(sessPref==='auto'
    ? 'OUTPUT ROUTING → AUTO — the browser chooses where sound goes. Try this if a connected speaker is being ignored.'
    : 'OUTPUT ROUTING → PLAYBACK — the app asks for the playback category explicitly.');
}
/* WRITE ONLY WHEN IT CHANGES.

   resumeSession is bound to touchstart, visibilitychange, pageshow and focus,
   and it ends in here — so the app was assigning navigator.audioSession.type
   on EVERY TOUCH, almost always to the value it already held. Assigning a
   category is not a read-modify-write of a variable: it is a request to the
   operating system to configure the audio session, and iOS re-evaluates the
   output route when a session is configured. Doing that under your finger,
   continuously, while audio is playing, is a good way to have the route
   re-picked — and the handset speaker is what it falls back to.
   Reported as sound coming out of the phone while a paired speaker is
   connected and other apps reach it fine, with a pad heard on Bluetooth and a
   lane started later heard on the phone: the same output, re-routed in
   between.
   I cannot confirm that from here — Chromium does not implement this API, so
   the line never executes in any test I can run. But writing a system setting
   thousands of times to the value it already has is wrong on its own terms,
   and the fix costs one comparison. */
let sessLast=null, sessWasOpen=false;
/* CLOSING THE LAST INPUT HAS TO RE-PICK THE OUTPUT, NOT JUST RELABEL IT.

   iOS chooses where audio goes when the audio session becomes active, and it
   does not revisit that choice because the category changed underneath it. So
   turning the microphone on moved output to the handset — correctly, record
   mode carries no A2DP — and turning it off set the category back to playback
   and left the sound exactly where it was. Every following build was fine
   until you touched the mic, and a RELOAD fixed it, which is the tell: a fresh
   context gets a fresh route.
   Confirmed by the person reporting it, from their own device: "I can restore
   the session and it plays through bt. Only sessions where I arm mic then turn
   it back off are affected."
   So the falling edge — the last input closing — rebuilds the output rather
   than assuming a relabel is enough. While nothing is playing that is the full
   context rebuild, which is what a reload does and is certain. While the
   transport is rolling it is the lighter output rebuild, because silently
   killing someone's playback to fix a routing nicety is not a trade worth
   making; RESET OUTPUT is there if the lighter one does not take. */
function applyAudioRoute(){
  const open=capturesOpen().length>0;
  const want = open ? 'play-and-record' : sessPref;
  if(want!==sessLast){
    let applied=false;
    try{ if(navigator.audioSession){ navigator.audioSession.type=want; applied=true; } }catch(e){}
    if(applied) sessLast=want;
  }
  if(sessWasOpen && !open) routeRepick();
  sessWasOpen=open;
  drawRoutePip();
  return open;
}
/* The claim side, so the release side knows there was something to release.
   The five capture sites used to set the category directly, which meant
   applyAudioRoute never saw the input open and never saw it close — so the
   falling edge that re-picks the route could not fire. One helper now marks
   the state as well as making the request. */
/* AudioContext.close() is ASYNCHRONOUS. A try/catch around it catches a
   synchronous throw and nothing else, so closing an already-closed context —
   which happens the moment two teardown paths overlap — escaped as an
   unhandled rejection: "Cannot close a closed AudioContext." Every close in
   the app goes through here. */
function closeCtx(c){
  if(!c) return;
  try{ if(c.state==='closed') return; const p=c.close(); if(p && p.catch) p.catch(()=>{}); }catch(e){}
}
function claimRecordRoute(){
  try{ if(navigator.audioSession){ navigator.audioSession.type='play-and-record'; sessLast='play-and-record'; } }catch(e){}
  sessWasOpen=true;
  drawRoutePip();
}
/* Written here and read by whoever reports the action, because routeRepick
   runs INSIDE micDisable and anything it says to the LCD is overwritten a
   moment later by the caller's own message. */
let routeNote='';
let repicking=false;
function routeRepick(){
  if(repicking || !AC || !LIVE) return;   // rebuildAudio calls back in here through disarm/disable
  repicking=true;
  try{
    if(!playing){
      const keep=sessPref;
      rebuildAudio();
      sessLast=null; sessPref=keep;
      try{ if(navigator.audioSession){ navigator.audioSession.type=sessPref; sessLast=sessPref; } }catch(e){}
      plog('Input closed — audio reopened so the phone picks an output again '
        +'(a category change alone does not move a route that is already running).');
      routeNote=' Audio was reopened, so a connected speaker gets the sound back.';
    } else {
      rebuildOut(true);
      plog('Input closed while the transport was rolling — the output was rebuilt without '
        +'interrupting playback. If sound is still coming from the phone, OUT ▸ RESET OUTPUT '
        +'does the full reopen.');
      routeNote=' Output rebuilt without stopping playback \u2014 if sound is still on the phone, use OUT \u25b8 RESET OUTPUT.';
    }
  }catch(e){ plog('Route re-pick failed: '+(e.message||e)); }
  repicking=false;
}
...
function stopMicStream(){
  ...
  if(meterCtx){ closeCtx(meterCtx); meterCtx=null; }
}
...
function exportSongDoc(){
  ...
  return { fmt:'mvx-songdoc-1', title:$('projName').value, bpm:S.bpm, swing:S.swing, human:S.human, silFade:S.silFade,
    ...
    takeSeed:S.takeSeed,   // the performance humanize/probability produce, so a reopened project bounces the same
...
/* ================= CLAUDE'S SONG — "Amber Signal" =================
   ...
   Sequenced with the app's own tools — pitch locks, ratchets, probability
   ghost notes, sidechain pump, humanize, ping-pong delay, and a SONG
   arrangement: intro ×2 → build ×2 → verse ×4 → chorus ×4 → verse ×2 →
   chorus ×4 → breakdown ×2 → outro ×2.

   WHAT IT USES THAT IT DID NOT BEFORE

   POLY. Two places, chosen because they are the two things a sixteen-step
   grid cannot write. In both choruses the closed hat keeps ordinary time
   except on steps 7 and 14, which open into three hits inside one beat —
   276 against a project at 92. Every other hat is marked PLAIN HIT, and
   that is not decoration: with the figure firing from all six lit hats it
   overlapped four deep and measured as hits 54ms apart across the whole
   bar. A machine gun, not a triplet. One property per step fixes it.
   In the breakdown the congas run three across the WHOLE bar — 69, three
   quarters of the tempo — against a kick squarely in four. Stripped back
   is the clearest place in the piece to hear three against four.

   NOTES. The warm pad held single notes, which wastes the one instrument
   in the kit that can carry a triad without turning to mud. It plays real
   chords now: Am and F in the verse, C and G in its answer.

   SOUNDS THAT ARE NOT IN THE PRESET LIST. The app ships no audio, so
   everything is synthesised — but a preset played straight is a preset.
   Two pads are built rather than chosen, by summing two renders with one
   arriving slightly late, which is what a player does with two layers on
   a pad:
     A11 GLASS PLUCK  pluck + a bell 12ms behind it. Struck together they
                      are one timbre; slightly apart the bell reads as the
                      string's own ring rather than a second instrument.
     A14 BOWED AIR    cello + glass, then played in GRAIN mode, so the pad
                      sprays fragments of the pair instead of playing them.
   A15 is the noise riser REVERSED — an intake before the chorus rather
   than a push into it. A12's bell drops an octave in the breakdown and
   holds its length, because KEEP TIME is on; tape pitch would have made
   that a short blip instead of a drone.

   Measured: 61.1s, 23 bars, peak 0.9315 with nothing at full scale, and
   0.984 correlation — it survives being folded to mono. */
/* MAKING A SOUND THAT IS NOT IN THE PRESET LIST.

   The app ships no audio, so every sound here is synthesised — but a preset
   played straight is a preset, and asked to be creative about it I would rather
   not just pick different ones. Two sounds in this song are built rather than
   chosen: two renders summed with one arriving slightly late, which is what a
   sampler player does with two layers on a pad, and it produces attacks and
   beating that neither source has on its own.

   Normalised on the way out, because a sum of two things that each peaked near
   1.0 does not, and a pad that arrives 6dB hotter than its neighbours is a
   mixing problem shipped as a feature. */
function blendBufs(a,b,opt){
  const o=opt||{};
  const ga=o.ga!=null?o.ga:1, gb=o.gb!=null?o.gb:1;
  const sr=a.sampleRate;
  const off=Math.max(0,Math.round((o.offset||0)*sr));
  const len=Math.max(a.length, b.length+off);
  const out=AC.createBuffer(2,len,sr);
  for(let ch=0;ch<2;ch++){
    const d=out.getChannelData(ch);
    const A=a.getChannelData(Math.min(ch,a.numberOfChannels-1));
    const B=b.getChannelData(Math.min(ch,b.numberOfChannels-1));
    for(let i=0;i<A.length;i++) d[i]+=A[i]*ga;
    for(let i=0;i<B.length;i++){ const j=i+off; if(j<len) d[j]+=B[i]*gb; }
  }
  let pk=0;
  for(let ch=0;ch<2;ch++){ const d=out.getChannelData(ch);
    for(let i=0;i<d.length;i++){ const v=Math.abs(d[i]); if(v>pk) pk=v; } }
  if(pk>0.99){ const g=0.99/pk;
    for(let ch=0;ch<2;ch++){ const d=out.getChannelData(ch);
...
async function loadClaudeSong(){
  ...
  try{
    ...
    const PADS=[ // [presetId, renderNote, tweaks] — pads A01–A16
      ...
      ['lpluck',69,{gain:.7,pan:-.2,rev:.3,dly:.25}],    // A4 pluck hook — layered below
      ['xbell',69,{gain:.5,pan:.3,rev:.5,dly:.2}],       // A4 bell accents
      ['dclap',39,{gain:.7,rev:.2}],
      ['xglass',57,{gain:.5,rev:.5}],                    // atmosphere — turned into a cloud below
      ['xsweep',60,{gain:.55,rev:.3,reverse:true}],      // REVERSED: an intake, not a riser
    ...
    /* ---- two sounds that are not in the preset list ---------------------- */
    const rp=async(id,note)=>renderPreset(PRESETS.find(p=>p.id===id),note);
    // A11 GLASS PLUCK — a pluck with a bell 12ms behind it. The offset is the
    // point: struck together they are one timbre, and slightly apart the bell
    // reads as the string's own ring rather than a second instrument.
    try{
      const gp=blendBufs(await rp('lpluck',69), await rp('xbell',81), {ga:1, gb:.5, offset:.012});
      assignBufToPad(10,gp,'glass pluck');
      Object.assign(S.pads[10],{gain:.7,pan:-.2,rev:.3,dly:.25});
    }catch(e){}
    // A14 BOWED AIR — cello and glass summed, then played in GRAIN mode, so the
    // pad sprays fragments of the pair instead of playing them. A texture that
    // exists nowhere in the preset list and cannot be got at by choosing one.
    try{
      const air=blendBufs(await rp('scello',45), await rp('xglass',57), {ga:.8, gb:.7, offset:.06});
      assignBufToPad(13,air,'bowed air');
      /* Density and burst kept modest on purpose. A grain cloud is the most
         expensive thing a pad can be — every grain is a source node — and this
         is the FACTORY song, which is the first thing a phone ever renders.
         14 grains/s over 1.6s is ~22 voices per hit; 9 over 1.1s is ~10, and at
         this spread and size it still reads as a cloud rather than a stutter.
         Caught by the engine suite picking up a real dropout on the machine
         running the tests once this pad went in. */
      Object.assign(S.pads[13],{gain:.5,rev:.55,dly:.15,mode:'grain',
        grSize:.16,grDens:9,grSpread:.35,grPitch:5,grPos:.18,grBurst:1.1});
    }catch(e){}
    // A12 the bell doubles as a drone: an octave down, holding its length
    // because KEEP TIME is on — tape pitch would have made it a short blip.
    Object.assign(S.pads[11],{keepTime:true});
    ...
    /* A CHORD is one step carrying several pitches — the NOTES lane's own
       shape. The warm pad held single notes before, which is a waste of the one
       instrument in the kit that can hold a triad without turning to mud. */
    const CH=(pi,pad,st,vel,pitches)=>{ P[pi].steps[pad][st]=vel;
      P[pi].locks[pad+':'+st]={pitches:pitches.slice(), pitch:pitches[0]}; };
    /* A POLY FIGURE. Its cells are fired by the pad's own lit steps, so the
       steps say WHEN and the figure says WHAT — three hits inside a beat, or
       three across a bar, neither of which sixteen steps can write.
       bpm is in the figure's own units: hits per minute. At a project tempo of
       92, 276 is three per beat and 69 is three per bar. */
    /* PLAIN HIT on a step: it plays one ordinary hit and skips the pad's
       figure. Without it a figure that lasts a beat, fired from hats a quarter
       of a beat apart, overlaps four deep and comes out as a machine gun —
       measured before this was added: hits 54ms apart across the whole bar. The
       part wants ordinary hats keeping time and two of them opening into
       triplets, which is one property per step. */
    const PLAIN=(pi,pad,steps)=>steps.forEach(st=>{
      const k=pad+':'+st, lk=P[pi].locks[k]||{};
      lk.plain=1; P[pi].locks[k]=lk;
    });
    const POLY=(pi,pad,bpm,cells,lit)=>{
      const row=polyEmptyRow();
      lit.forEach(([i,v])=>{ row[i]=v; });
      P[pi].poly=P[pi].poly||{};
      P[pi].poly[pad]={on:true,bpm,cells,lock:true,trig:'step',row};
    };
    ...
    N(1,14,8,.5);
    // P2 VERSE Am|F — the pad holds real triads now, not one note
    ...
    /* THE TRIPLET HATS. Two steps in the bar stop being single hats and become
       three-inside-a-beat, against a kick that is squarely in four. This is the
       figure a sixteen-step grid cannot write at all — sixteen does not divide
       by three — and it is why POLY exists. */
    POLY(4,2,276,3,[[0,.5],[1,.32],[2,.42]]);
    PLAIN(4,2,[0,4,6,8,12,15]);      // the timekeeping hats stay single hits
    N(4,2,7,.5); N(4,2,14,.5);       // these two open into a triplet
    ...
    POLY(5,2,276,3,[[0,.5],[1,.32],[2,.42]]);
    PLAIN(5,2,[0,4,6,8,12,15]);
    N(5,2,3,.5); N(5,2,7,.5); N(5,2,14,.5);
    ...
    /* THREE AGAINST FOUR, across the whole bar. One step at the top of the bar
       fires a three-cell figure at 69 — three quarters of the project tempo —
       so the congas lay three evenly over the kick's four. With the drums
       stripped back it is the clearest place in the song to hear it. */
    POLY(6,15,69,3,[[0,.6],[1,.42],[2,.5]]);
    N(6,15,0,.6);
    /* The bell an octave down, holding its length because KEEP TIME is on. Tape
       pitch would have made this a short blip instead of a drone. */
    N(6,11,0,.4,-12); N(6,11,8,.35,-12);
    ...
    buildPads(); drawPads(); drawEdit(); drawSeq(); drawFader(); drawMixer(); drawSong(); dirty();
    lcd("CLAUDE'S SONG · “Amber Signal” — press PLAY. New in this version: triplet hats "
      +"in the chorus and three-against-four congas in the breakdown (SEQ ▸ POLY), real chords "
      +"on the warm pad, and two sounds built by layering two renders — A11 glass pluck, "
      +"A14 bowed air (a GRAIN cloud). ♪");
...
function polyHitsForStep(cfg, absStep, t, sd){
  ...
  /* Anchor to the CYCLE, not the bar. A rate of 3.5 hits per bar only returns
     to a downbeat every two bars, so re-anchoring every bar forced a hit onto
     every downbeat and corrupted the pattern — a converted 7-over-2-bars came
     out with a spurious hit at 2.4s. polyCycleBars says how long the cycle
     actually is. */
  const cycB=polyCycleBars(cfg, barDur) || 1;
  const cycSteps=NSTEPS*cycB;
  const cycStart=t-posMod(absStep,cycSteps)*sd;
  const hitsPerCycle=Math.round(polyLockedHitsPerBar(cfg, barDur)*cycB);
  const cycIdx=Math.floor(absStep/cycSteps);
  return polyEvents(cfg, cycStart, cycIdx*hitsPerCycle, t, t+sd, barDur);
}
...
function startSeq(){
  ensureAudio();
  if(playing) return;
  try{ traxPreviewStop(); }catch(e){}   // an audition is not part of the song
  playing=true;
  glitchReset();          // the reading is always "dropouts while you were listening"
  restoreTails();         // a drain from the last STOP must not eat the first bars
...
let tailRestoreT=0;
/* PLAY cancels a drain in progress and restores immediately. Without this,
   pressing play within 2.1s of stop started the song with its sends at zero and
   faded them up underneath it — the effects arriving late, which is exactly as
   wrong as the tail arriving late was. */
function restoreTails(){
  clearTimeout(tailRestoreT); tailRestoreT=0;
  if(!AC||!LIVE) return;
  const t=AC.currentTime;
  const back=(p,v)=>{ try{ p.cancelScheduledValues(t); p.setValueAtTime(v,t); }catch(e){} };
...
function stopSeq(){
  ...
  /* Only STOP drains the tails. It was hung off panicVoices in R154, which is
     also called by CLR PTN and by loading a project — so clearing a pattern
     mid-song ducked the reverb and delay sends for two seconds and let them
     swell back in. Reported as "my playback is messed up, it has a distortion
     that's not from effects". Wrong hook; the intent was always "STOP means
     silence", not "any panic means silence". */
...
function ribbonStart(){
  ...
  subo.connect(sg); sg.connect(lp); lp.connect(g); g.connect(bus.in);
...
function fluteVoiceStart(f){
  ...
  const g=AC.createGain(); g.gain.value=0;
  o.connect(g); o2.connect(g2); g2.connect(g); nz.connect(nf); nf.connect(ng); ng.connect(g); g.connect(bus.in);
...
function breathStop(){
  ...
  if(breathCtx){ closeCtx(breathCtx); breathCtx=null; }
...
function hitPerc(v){
  ...
  function burst(dur,f,q,g0,at){
    ...
    n.connect(bp); bp.connect(g); g.connect(bus.in); n.start(at||t);
  ...
  function jingles(n,base,spread,dec,g0){
    for(let i=0;i<n;i++){
      ...
      o.connect(g); g.connect(bus.in); o.start(t0); o.stop(t0+dec+0.3);
...
/* Every voice has a SHAPE now, and every one of them names the actual
   parameter rather than saying "brightness". Three of the seven had one; the
   other four had no control at all, which is what "there's no settings to
   customize the keyboard sound" was about. */
const SHAPE_WHAT={
  ...
  glass:'how far the two tones disagree. At zero they fuse into one clean bell; wide open they '
    +'beat against each other and the note shimmers.',
  saw:'the unison spread between the two saws. Narrow is one firm saw, wide is the slow '
    +'chorusing a supersaw is made of.',
...
/* The four tone controls read out in the units they actually are, not in
   percent. A cutoff that says "62%" tells you nothing you can act on; one that
   says 4.2kHz can be matched to what you are hearing. The envelope pair reads
   as a multiplier because that is honestly what it is \u2014 "\u00d71" means this voice
   as it comes, and a number of milliseconds would be a different lie for each
   of the seven. */
function drawInstTone(){
  const set=(id,val,txt)=>{ const el=$(id); if(el) el.value=val;
    const vv=$(id+'V'); if(vv) vv.textContent=txt; };
  const hz=instCutHz();
  set('instCut', S.inst.cut==null?1:S.inst.cut,
    (S.inst.cut>=0.999?'open':(hz>=1000?(hz/1000).toFixed(hz>=10000?0:1)+'kHz':Math.round(hz)+'Hz')));
  const q=instResQ();
  set('instRes', S.inst.res||0, (S.inst.res>0.001?'Q '+q.toFixed(1):'off'));
  const am=instAtkMul(), rm=instRelMul();
  ...
  /* The whole point of a filter is that you hear it move, so it moves under a
     held note rather than waiting for the next one. */
  if(LIVE && LIVE._inst){
    const t=AC.currentTime;
    try{ LIVE._inst.flt.frequency.setTargetAtTime(hz,t,0.02);
...
/* WHAT YOU SEE IS WHAT PLAYS, AND WHAT GETS SAVED.
   Shortening a track or a pattern used to hide the steps past the new end
   without removing them. They stopped sounding, so the grid and the playback
   still agreed — but they were still in the pattern, still written into every
   save, and the moment the length grew again they came back. Delete a hit,
   shorten, lengthen, and the hit you deleted is playing again in its original
   place: the app appearing to overrule you.
   Trimming is destructive on purpose. A row that is eight steps long contains
   eight steps, and the file contains what the grid shows. */
function trimTrack(pat,pad,L){
  const row=pat.steps[pad]; if(!row) return 0;
  let cut=0;
  for(let i=L;i<row.length;i++){ if(row[i]>0){ row[i]=0; cut++; } }
  if(pat.locks) for(let i=L;i<MAXSTEPS;i++) delete pat.locks[pad+':'+i];
  return cut;
}
function setTrackLen(d){
  ...
  const cut=trimTrack(pat,S.seqPad,pat.len[S.seqPad]);
  ...
  lcd('TRACK '+padName(S.seqPad)+' LEN '+pat.len[S.seqPad]+(pat.len[S.seqPad]!==PL?' · polymeter':'')
    +(cut?' — '+cut+' hit'+(cut>1?'s':'')+' past the end removed, not hidden':''));
}
function setPatLen(n){
  ...
  let cut=0;
  for(let p=0;p<NPADS;p++) cut+=trimTrack(pat,p,pat.len[p]);
  if(pat.sil) for(let i=n;i<pat.sil.length;i++) pat.sil[i]=0;
...
/* STORED AUDIO: 16-bit PCM, scaled per channel.

   Every persistence path in the app — the .json project file, the project
   library, and the autosave vault the app restores from after a crash — kept
   audio as Int16 clamped to +/-1:

       i16[i] = clamp(round(f[i] * 32767))

   Two things were wrong with that, and they pull in opposite directions.

   IT CLIPPED. The TRAX capture tap sits at perfGain, upstream of the limiter,
   deliberately — moving it downstream would re-record playing lanes into every
   overdub. So a take can legitimately exceed 1.0, and the float path carries it
   happily; 32-bit-float export was measured carrying samples at 1.9 intact.
   Saving flat-topped every one of those, permanently, including into the slot
   crash-recovery reads. A recorded take peaking 1.163 lost 89 samples; a
   sustained tone at 1.16 lost 16,280, which is audible flat-topping.

   IT WASTED THE QUIET END. The error of an undithered 16-bit grid is fixed in
   absolute terms, so it is the quiet material that suffers: measured through
   the real vault, -6dBFS kept 15.0 effective bits and -60dBFS kept 6.0.

   One scale factor per channel fixes both. The channel is normalised to full
   scale on the way in and multiplied back on the way out, so nothing clips at
   any level and every channel gets the whole 16-bit grid regardless of how
   quiet it is. It costs one float per channel and not one byte of sample data.

   Absent pk means 1, which is exactly the old behaviour, so documents written
   before this still decode correctly. DOC_V goes to 3 so that an OLDER build,
   which would ignore pk and play a normalised take at the wrong level, refuses
   the file instead of misreading it. */
function pcmPeak(f32){
  let pk=0;
  for(let i=0;i<f32.length;i++){ const a=Math.abs(f32[i]); if(a>pk) pk=a; }
  return (pk>0 && isFinite(pk)) ? pk : 1;      // silence scales by 1, not by 0
}
function pcmQuant(f32,pk){
  const i16=new Int16Array(f32.length), inv=32767/pk;
  for(let i=0;i<f32.length;i++){
    const v=Math.round(f32[i]*inv);
    i16[i] = v>32767 ? 32767 : (v<-32768 ? -32768 : v);   // rounding only, the scale did the rest
  }
  return i16;
}
/* PK_MAX: a scale factor is the peak of a recorded channel, so anything past a
   few times full scale is a damaged or hostile document rather than a loud
   take. Unclamped, pk:1e30 was accepted and produced samples around 9.4e29 in
   a live AudioBuffer. The limiter and soft clip absorbed it — measured 0.853
   against a 0.843 baseline with no non-finite samples, even with reverb and
   delay feedback up — so this is hardening rather than a fix for an audible
   fault. It costs one comparison and removes the need to have been lucky. */
const PK_MAX=64;
function pcmScale(pk){ return (pk>0 && isFinite(pk)) ? Math.min(pk,PK_MAX) : 1; }
function pcmRestore(i16,pk){
  const k=pcmScale(pk)/32767, f=new Float32Array(i16.length);
  for(let i=0;i<i16.length;i++) f[i]=i16[i]*k;
  return f;
}
function f32ToB64(f32,pk){
  const i16=pcmQuant(f32,pcmScale(pk));
  const u8=new Uint8Array(i16.buffer);
  let s=''; const CH=0x8000;
  for(let i=0;i<u8.length;i+=CH) s+=String.fromCharCode.apply(null,u8.subarray(i,i+CH));
  return btoa(s);
}
function b64ToF32(b64,pk){
  const bin=atob(b64), u8=new Uint8Array(bin.length);
  for(let i=0;i<bin.length;i++) u8[i]=bin.charCodeAt(i);
  return pcmRestore(new Int16Array(u8.buffer), pk);
}
...
$('btnSave').addEventListener('click',()=>{
  ...
  const bufs=S.buffers.map(b=>{
    const chans=[], pk=[];
    for(let c=0;c<b.numberOfChannels;c++){
      const d=b.getChannelData(c), p=pcmPeak(d);
      pk.push(p); chans.push(f32ToB64(d,p));
    }
    return {sr:b.sampleRate,len:b.length,ch:chans,pk};
  });
...
/* Every numeric field that ends up in an AudioParam or a clamp that assumes a
   number. docAccept refuses a document that carries any of these as something
   other than a finite number, so the apply path cannot throw half way. */
const DOC_NUMS=['bpm','swing','human','masterVol','delayFb','silFade','revLvl','revSize',
  'delayDiv','dlyTone','compAmt','midiCh','midiRoot','midiOutCh','scaleRoot',
  'mEqLo','mEqMid','mEqHi','mWidth','mMono','mCeil','mTrim',
  'mMud','mAir','mFocus','mDeess','scDepth','scRel','takeSeed'];
const DOC_V=3;                 // schema version — bump ONLY when a field changes meaning
...
function migrateDoc(doc,from){
  ...
  /* v2 → v3: stored audio carries a per-channel scale. Nothing to rewrite —
     a v2 buffer was quantised against full scale, which is exactly what an
     absent pk means, so the decoders read it correctly as it stands. The bump
     exists for the other direction: a v2 build would ignore pk and play every
     normalised channel at the wrong level, so it must refuse the file rather
     than open it and be quietly wrong. */
  if(from<3){ /* v2 → v3: no stored field changed meaning */ }
  doc.v=DOC_V;
  return doc;
}
...
function docAccept(doc,where){
  ...
  /* Numbers that reach an AudioParam, checked before anything is applied.
     An AudioParam throws on a non-finite value, and the apply path is a long
     straight run of assignments with no transaction around it — so one bad
     number a third of the way down left pads, patterns and tape lanes
     replaced and everything after them untouched, under a message saying
     nothing had loaded. That is the exact failure the note at the top says
     must not happen, so it is caught here rather than in each assignment.
     Present-and-wrong is refused; absent is fine, the loader has defaults.
     Last, after the version gate, so a document from a newer build keeps its
     own more useful message instead of being called damaged. */
  for(const k of DOC_NUMS){
    const x=doc[k];
    if(x!=null && (typeof x!=='number' || !isFinite(x))){
      lcd('LOAD FAILED — that project file is damaged.');
      plog('Refused '+where+': "'+k+'" is '+JSON.stringify(x)+', which is not a finite number. '+
           'It would be written to an audio parameter part way through loading and throw, '+
           'so nothing in this session was changed.');
      return false;
    }
  }
  /* And the same one level down, where DOC_NUMS could not reach.

     pads[].gain, .pan, .rev and .dly go to hardSet and then to an AudioParam,
     so a pad carrying a string broke the invariant exactly as masterVol did —
     and worse, because it did not stop at the session: the poisoned pad
     reached S, autosave wrote it to the vault, and RESTORE brought it back,
     leaving S.pads[0].gain a string while the audio node still held 0.9. The
     state and the graph disagreed permanently and nothing said so.

     The list is not written out here. newPad(0) and newTrack() ARE the schema,
     so every field that is a number in the reference must be a finite number
     here if it is present at all. That is the lesson from DOC_NUMS, which had
     to be corrected twice for naming fields that were not numbers: a check
     derived from the shape cannot drift away from the shape. */
  const shapeBad=(list,ref,what)=>{
    if(!Array.isArray(list)) return null;
    for(let i=0;i<list.length;i++){
      const o=list[i];
      if(o==null || typeof o!=='object' || Array.isArray(o)) return what+'['+i+'] is '+JSON.stringify(o);
      for(const k in ref){
        if(typeof ref[k]!=='number') continue;
        const x=o[k];
        if(x!=null && (typeof x!=='number' || !isFinite(x)))
          return what+'['+i+'].'+k+' is '+JSON.stringify(x);
      }
    }
    return null;
  };
  const bad = shapeBad(doc.pads,newPad(0),'pads') || shapeBad(doc.trax,newTrack(),'trax');
  if(bad){
    lcd('LOAD FAILED — that project file is damaged.');
    plog('Refused '+where+': '+bad+', which is not a finite number. It would be written to an '+
...
function applySessionDoc(doc, bufs){
  ...
  S.bpm=(doc.bpm!=null)?doc.bpm:120; S.swing=(doc.swing!=null)?doc.swing:0; S.human=doc.human||0;
  ...
  S.takeSeed = (doc.takeSeed!=null) ? doc.takeSeed : newTakeSeed();
  ...
  S.autoWarp=false;
  S.silFade=doc.silFade!=null?doc.silFade:0.06;
  /* These two go straight into AudioParams a few hundred lines down, and an
     AudioParam throws on a non-finite value — so a document missing either one
     used to fail PART WAY THROUGH applySessionDoc, after S.pads, S.patterns
     and S.trax had already been replaced. The user got "LOAD FAILED" over a
     session that had in fact been half-overwritten and could not be got back.
     docAccept now refuses such a file at the gate, and these carry defaults so
     absence is a value rather than a fault. Both belts, because the invariant
     in docAccept says every route in is refused whole or applied whole, and a
     field that defends itself is one less way to break that later. */
  S.masterVol=(doc.masterVol!=null)?doc.masterVol:0.9;
  S.delayFb=(doc.delayFb!=null)?doc.delayFb:0.35;
  ...
  /* Defaults taken from S's own initial values, not invented: vcurve is the
     string 'linear' and midiCh is -1 (omni), so 0.5 and 0 would have been
     quietly wrong rather than absent. */
  S.vcurve=(doc.vcurve!=null)?doc.vcurve:'linear'; S.midiCh=(doc.midiCh!=null)?doc.midiCh:-1;
  S.pcPat=(doc.pcPat!=null)?doc.pcPat:true; S.ccMaps=doc.ccMaps||{};
...
$('jsonIn').addEventListener('change',async e=>{
  ...
  try{
    ...
    const bufs=doc.buffers.map(b=>{
      ...
      b.ch.forEach((c64,i)=>ab.copyToChannel(b64ToF32(c64, b.pk?b.pk[i]:1),i));
      return ab;
    });
...
function snapshotSession(){
  ...
  const bufs=S.buffers.map(b=>{
    const ch=[], pk=[];
    for(let c=0;c<b.numberOfChannels;c++){
      const f=b.getChannelData(c), p=pcmPeak(f);
      pk.push(p); ch.push(pcmQuant(f,p).buffer);
    }
    return {sr:b.sampleRate,len:b.length,ch,pk};
  });
...
let emptyStashT=-1;      // the vault timestamp already filed away by the empty-session guard
...
async function autosave(){
  ...
  /* Opening the app schedules an autosave of its own — boot calls setBpm, which
     calls dirty() — so this fires about 1.5s after launch with nobody having
     touched anything. That is what used to overwrite the offered session before
     you could reach RESTORE, and after R123 it filed it into REWIND and took
     the offer off screen instead: better, but the bar still vanished by itself.
     While a real session is on offer and the live one holds no audio, there is
     nothing here worth writing. Leave the offer alone and wait for you to
     actually make something. */
  if(pendingRestore && !S.buffers.length) return;
  /* An EMPTY session must not overwrite a vault that holds audio.

     Undoing to the bottom returns the session to its pre-load state — no
     buffers, no loaded pads — and dirty() then fires an ordinary autosave, so
     the vault went from sixteen buffers to zero. Redo still held the work, but
     the undo stack lives in memory only: lose the page in that window (a
     crash, an iOS tab reap, a closed lid) and redo goes with it while the
     vault holds an empty session. The work is gone, silently, which is the one
     outcome this file consistently refuses to allow.
     The vault exists to recover work, and there is no work here to recover, so
     the old one is filed into REWIND and the slot is left alone. Filed once
     per distinct vault, so an genuinely empty session does not churn out a
     checkpoint every time something is touched. */
  if(!S.buffers.length){
    try{
      const prev=await idbGet('last');
      if(prev && Array.isArray(prev.buffers) && prev.buffers.length){
        const stamp=prev.t||0;
        if(emptyStashT!==stamp){
          emptyStashT=stamp;
          try{ await idbPut('ckpt:'+(stamp||Date.now()), prev); }catch(e){}
          plog('This session holds no audio, and the autosave slot held '+prev.buffers.length+
               ' sample'+(prev.buffers.length===1?'':'s')+' — so the slot was left alone and that '+
               'session was filed into REWIND. Undo does not live on disk; redo it, or take it '+
               'back from PROJ \u25b8 REWIND.');
          lcd('AUTOSAVE HELD BACK — this session is empty, so the saved one was kept and copied to REWIND.');
        }
        return;
      }
    }catch(e){}
...
function docToBuffers(doc){   // PCM (Int16) back to AudioBuffers
  return (doc.buffers||[]).map(b=>{
    const ab=mkAudioBuf(b.len,b.sr,b.ch.length);
    b.ch.forEach((raw,i)=>{
      ab.copyToChannel(pcmRestore(new Int16Array(raw), b.pk?b.pk[i]:1), i);
    });
    return ab;
  });
}
async function offerRestore(){
  ...
  // nothing is loading in this branch — it waits for you, so it really is ready
...
/* ---------------- LOUDNESS AND TRUE PEAK, PROPERLY ----------------
   The panel's LOUDNESS number was a K-weighted RMS of ONE analyser frame — 46ms
   of a mono downmix, ungated, through approximate filters — with "LOUDNESS"
   written over it next to PEAK and RMS, which are true. Measured against a real
   BS.1770 implementation on the same bounce it read -6.4 where the answer was
   -11.1: 4.6dB optimistic. Aim a master at -14 with that and you deliver -18.6.

   What follows is the actual thing. The K-weighting is the specified pair — a
   +3.999dB shelf at 1681.97Hz and an RLB highpass at 38.135Hz — derived at
   whatever rate the context is running rather than hard-coded for 48k. Blocks
   are 400ms at 75% overlap, each channel's mean square summed with weight 1.0,
   and the two gates applied: absolute at -70 LUFS, then relative at 10 LU below
   the ungated mean.

   True peak is a polyphase 4x windowed-sinc interpolation. Two earlier attempts
   returned exactly the sample peak — one from a mis-windowed kernel, one from
   the browser's own resampler, which is linear and so can never overshoot. This
   one is validated against a sine at sr/4 sampled on its zero crossings, where
   the samples sit at -3.01dB and the true peak is 0.00dB by construction. */
function k1770Coeffs(sr){
  const shelf=(()=>{ const f0=1681.974450955533, G=3.999843853973347, Q=0.7071752369554196;
    const K=Math.tan(Math.PI*f0/sr), V=Math.pow(10,G/20), den=1+K/Q+K*K;
    return { b0:(V+Math.sqrt(V)*K/Q+K*K)/den, b1:2*(K*K-V)/den, b2:(V-Math.sqrt(V)*K/Q+K*K)/den,
             a1:2*(K*K-1)/den, a2:(1-K/Q+K*K)/den }; })();
  const hp=(()=>{ const f0=38.13547087602444, Q=0.5003270373238773;
    const K=Math.tan(Math.PI*f0/sr), den=1+K/Q+K*K;
    return { b0:1/den, b1:-2/den, b2:1/den, a1:2*(K*K-1)/den, a2:(1-K/Q+K*K)/den }; })();
  return {shelf,hp};
}
function biquadRun(x,c,outArr){
  const y=outArr||new Float64Array(x.length);
...
/* Integrated loudness of a finished buffer, to the standard. */
function lufsIntegrated(buf){
  const sr=buf.sampleRate, c=k1770Coeffs(sr), chans=[];
  for(let ch=0; ch<buf.numberOfChannels; ch++)
    chans.push(biquadRun(biquadRun(buf.getChannelData(ch),c.shelf),c.hp));
  const T=Math.round(sr*0.4), step=Math.max(1,Math.round(T/4));
  const lj=[], zj=[];
  for(let s=0; s+T<=chans[0].length; s+=step){
    let z=0;
    for(const d of chans){ let m=0; for(let i=s;i<s+T;i++) m+=d[i]*d[i]; z+=m/T; }
    zj.push(z); lj.push(-0.691+10*Math.log10(z||1e-30));
  }
  if(!lj.length) return {lufs:-Infinity, blocks:0, kept:0, momentaryMax:-Infinity};
  const meanOf=idx=>idx.reduce((a,i)=>a+zj[i],0)/idx.length;
  let idx=zj.map((_,i)=>i).filter(i=>lj[i]>-70);
  if(!idx.length) return {lufs:-Infinity, blocks:lj.length, kept:0, momentaryMax:Math.max.apply(null,lj)};
  const rel=-0.691+10*Math.log10(meanOf(idx))-10;
  idx=idx.filter(i=>lj[i]>rel);
  if(!idx.length) return {lufs:-Infinity, blocks:lj.length, kept:0, momentaryMax:Math.max.apply(null,lj)};
  return { lufs:-0.691+10*Math.log10(meanOf(idx)), blocks:lj.length, kept:idx.length,
           momentaryMax:Math.max.apply(null,lj) };
}
/* Polyphase 4x true peak. Built once, reused. */
const TP_OS=4;
const TP_PHASES=(function(){
  const TAPS=24, half=TAPS*TP_OS/2, n=TAPS*TP_OS+1, proto=new Float64Array(n);
  for(let i=0;i<n;i++){
    const x=(i-half)/TP_OS;
    const s=x===0?1:Math.sin(Math.PI*x)/(Math.PI*x);
    const w=0.42-0.5*Math.cos(2*Math.PI*i/(n-1))+0.08*Math.cos(4*Math.PI*i/(n-1));
    proto[i]=s*w;
  }
  const ph=[];
  for(let q=0;q<TP_OS;q++){
    const br=[]; for(let i=q;i<n;i+=TP_OS) br.push(proto[i]);
    const sum=br.reduce((a,v)=>a+v,0);
    ph.push(br.map(v=>v/sum));
  }
  ph.offset=Math.floor(half/TP_OS);
  return ph;
})();
function truePeakOf(buf){
  /* Interpolating every sample is 4 phases x 25 taps per sample — fine for a
     six-second loop, over a billion operations for a three-minute song, on a
     phone. An inter-sample peak can only sit between samples that are already
     near the top, so find the sample peak first and only interpolate around
     the samples within 3dB of it. Exact where it matters, and hundreds of
     times less work.
     The ends are skipped by a kernel's width: outside the buffer the
     interpolator sees zeros, so the first and last samples read as a step, and
     a band-limited interpolation of a step overshoots by about 1dB. That is an
     artifact of where the file stops, not of the audio. */
  const edge=TP_PHASES[0].length, off=TP_PHASES.offset;
  let pk=0;
  for(let ch=0; ch<buf.numberOfChannels; ch++){
    const d=buf.getChannelData(ch);
    const lo=Math.min(edge,Math.floor(d.length/2)), hi=Math.max(lo,d.length-edge);
    let sp=0;
    for(let i=lo;i<hi;i++){ const a=d[i]<0?-d[i]:d[i]; if(a>sp) sp=a; }
    if(sp<=0) continue;
    if(sp>pk) pk=sp;                       // the sample peak is a floor for the true peak
    const thr=sp*0.708;                    // within 3dB of the loudest sample
    for(let n=lo;n<hi;n++){
      const a0=d[n]<0?-d[n]:d[n];
      if(a0<thr) continue;
      for(let q=0;q<TP_OS;q++){
...
function samplePeakOf(buf){
  let pk=0;
  for(let ch=0; ch<buf.numberOfChannels; ch++){ const d=buf.getChannelData(ch);
    for(let i=0;i<d.length;i++){ const a=Math.abs(d[i]); if(a>pk) pk=a; } }
  return pk;
}
/* What the finished file actually is, in the terms a release is judged on. */
function bounceReport(buf){
  const li=lufsIntegrated(buf);
  const sp=samplePeakOf(buf), tp=truePeakOf(buf);
  const d=v=>v<=0?'-inf':(20*Math.log10(v)).toFixed(2);
  const lines=[];
  lines.push('Integrated loudness '+(li.lufs===-Infinity?'—':li.lufs.toFixed(1)+' LUFS')
    +'  ·  true peak '+d(tp)+' dBTP  ·  sample peak '+d(sp)+' dBFS');
  if(li.lufs>-Infinity){
    const vs=li.lufs+14;
    lines.push('Streaming sits near -14 LUFS: this is '
      +(Math.abs(vs)<0.5?'right on it.':(vs>0?vs.toFixed(1)+'dB louder — platforms will turn it down.'
                                             :Math.abs(vs).toFixed(1)+'dB quieter — platforms will leave it, it will just sound smaller.')));
  }
  if(20*Math.log10(tp)>S.mCeil+0.05)
    lines.push('True peak is '+(20*Math.log10(tp)-S.mCeil).toFixed(2)+'dB above the '+S.mCeil
      +'dB CEILING — the limiter is 20:1, not a brickwall, so it lets a little through.');
...
/* ---------------- THE TAKE ----------------
   HUMANIZE and per-step PROBABILITY are supposed to vary — that is the whole
   point of them. What is NOT reasonable is that bouncing the same project twice
   gave two different performances with nothing saying so: you export, listen,
   like it, export again to fix a typo in the filename, and get a different
   record. Measured on the demo project that difference was -11dB RMS. Not
   subtle — a different take.
   So the render draws from a SEEDED generator instead. The seed is part of the
   project, so the same project bounces to the same performance today and next
   year, and NEW TAKE rolls a fresh one when a different feel is what you want.
   Live playback is deliberately left alone: a jam should breathe.
   (Underneath this there is a residual around -66dB from the browser summing
   more than about five nodes in an order it does not promise to repeat. Float
   addition is not associative, so a different order changes the last bit. It
   is inaudible, it is not ours, and no amount of seeding touches it.) */
let takeRnd = Math.random;
function newTakeSeed(){ return (Math.random()*0x7fffffff)|0; }
function takeSeedStart(){
  if(S.takeSeed==null) S.takeSeed=newTakeSeed();
  takeRnd = mulberry32(S.takeSeed);
}
function takeSeedEnd(){ takeRnd = Math.random; }
function drawTake(){
  const el=$('bTake'); if(!el) return;
  if(S.takeSeed==null) S.takeSeed=newTakeSeed();
  el.textContent='TAKE #'+String(S.takeSeed>>>0).toString(36).toUpperCase();
  el.title='The performance humanize and probability produce. The same seed always '
    +'bounces the same take; NEW TAKE rolls a different one.';
}
async function renderMix(padSet, traxSet, opt){
  takeSeedStart();
  try{ return await renderMixInner(padSet, traxSet, opt); }
  finally{ takeSeedEnd(); }
}
async function renderMixInner(padSet, traxSet, opt){
        ...
        for(let p=0;p<NPADS;p++){
          ...
          if(pc && polyTrig(pc)==='free'){
            ...
            if(!polyDoesLock(pc,barDur)){ t0p=renderT0; idx0=0; }
              ...
              const cycB=polyCycleBars(pc,barDur)||1, cycSteps=NSTEPS*cycB;
          ...
          const v=pat.steps[p][idx]; if(!(v>0)) continue;
          const lk=pat.locks&&pat.locks[p+':'+idx];
          if(!worst && lk && lk.prob!=null && takeRnd()>lk.prob) continue;
...
$('btnBounce').addEventListener('click',async ()=>{
  ...
  try{
    ...
    let rep=[];
    try{ rep=bounceReport(rendered); rep.forEach(l=>plog(l)); }catch(e){ plog('Could not measure the bounce: '+e.message); }
    lcd('BOUNCE COMPLETE · '+(rep[0]||''));
...
let lastDiag='';
$('btnDiag').addEventListener('click',()=>{ lastDiag=diagDump('manual'); $('docText').value=lastDiag;
  lcd('DIAG in the text box + PROJ log — tap COPY to put it on the clipboard.'); });
/* Asked for directly: "an easy way to copy and paste the diagnostic report
   would help". Selecting a full dump by hand on a phone is genuinely awkward,
   and a report that is hard to send is a report that does not get sent.
   The textarea fallback is there because clipboard.writeText needs a secure
   context and a recent gesture, and neither is guaranteed on every phone —
   select-all at least leaves one long-press between you and a paste. */
$('btnDiagCopy').addEventListener('click',async ()=>{
  const d=lastDiag||diagDump('manual'); lastDiag=d; $('docText').value=d;
  try{
    if(!navigator.clipboard || !navigator.clipboard.writeText) throw new Error('no clipboard API');
    await navigator.clipboard.writeText(d);
    lcd('DIAG COPIED — paste it straight into a bug report.');
  }catch(e){
    try{ $('docText').focus(); $('docText').select(); }catch(e2){}
    lcd('COULD NOT REACH THE CLIPBOARD — the report is selected in the box below, long-press \u2192 Copy.');
  }
});
...
function rebuildOut(quiet){
  ...
  try{
    ...
    if(!quiet) lcd('OUT REVIVED · '+AC.state);
  }catch(e){
    try{ LIVE.softclip.connect(AC.destination); }catch(e2){}
  }
}
function rebuildAudio(){ // last resort: wedged context — new context, new graph, same session
  ...
  /* close() is ASYNCHRONOUS and returns a promise. A try/catch around it
     catches a synchronous throw and nothing else, so closing an already-closed
     context — which happens the moment two rebuild paths overlap, and there
     are now several — surfaced as an unhandled rejection: "Cannot close a
     closed AudioContext." Caught properly, and the reference is dropped first
     so a re-entrant call has nothing to close twice. */
  const dying=AC; AC=null; closeCtx(dying);
  LIVE=null; bbTap=null;   // black box re-arms on the fresh context (history kept if the rate matches)
  liveVoices.clear();
  Object.keys(chokeLive).forEach(k=>delete chokeLive[k]);
  Object.keys(activeEnv).forEach(k=>delete activeEnv[k]);
  ensureAudio();
...
function diagDump(tag){
  try{
    ...
    const lines=[
      ...
      // The computed delay time alone is unreadable: 1.900s is also the clamp
      // ceiling, so it could be a slow tempo or a bad division being capped and
      // there was no way to tell them apart from a report.
      (function(){ const raw=60/bpmAbs()*S.delayDiv;
        return 'delay '+delayTime().toFixed(3)+'s'+(raw>1.9?' ⚠CLAMPED from '+raw.toFixed(2)+'s':'')
      ...
      (function(){ let red='-'; try{ red=LIVE?LIVE.limiter.reduction.toFixed(1)+'dB':'-'; }catch(e){}
        return 'master — ceiling '+S.mCeil+'dB trim '+S.mTrim+'dB · limiter reduction now '+red
          +' · vol '+S.masterVol+' · pads over 100%: '
          +(S.pads.map((pd,i)=>pd.bufId>=0&&pd.gain>1?padName(i)+':'+Math.round(pd.gain*100)+'%':null).filter(Boolean).join(' ')||'none'); })(),
      'buffers:'+S.buffers.length+' loadedPads:'+S.pads.filter(x=>x.bufId>=0).length+' dangling:'+(dang.length?dang.join(' '):'none')+' voices:'+acts,
      /* WHETHER THERE IS ANY AUDIO IN THEM. A report came in of silence with a
         DIAG that looked perfect — running context, gates open, pad unmuted,
         no dangling ids — because nothing here said whether the one buffer
         held sound. It did not. A dump that cannot distinguish "healthy and
         playing nothing" from "healthy and holding silence" sends everyone
         looking at the graph, which is the one part that was fine. */
      'buffer peaks: '+(S.buffers.length
        ? S.buffers.map((b,i)=>{ if(!b) return i+':MISSING';
            let pk=0; const d=b.getChannelData(0);
            for(let k=0;k<d.length;k+=32){ const a=Math.abs(d[k]); if(a>pk) pk=a; }
            const on=S.pads.map((pd,pi)=>pd.bufId===i?padName(pi):null).filter(Boolean)
              .concat(S.trax.map((tr,ti)=>tr.bufId===i?'T'+(ti+1):null).filter(Boolean));
      ...
      'input streams: '+(liveInputTracks().join(' ')||'none live')
        ...
        +' · routing pref '+sessPref+' · last asked '+(sessLast==null?'never':sessLast)
        +' · audioSession API '+((()=>{ try{ return navigator.audioSession?'present':'ABSENT'; }catch(e){ return 'ABSENT'; } })()),
      'lanes: '+(S.trax.some(t=>t.bufId>=0)
        ? S.trax.map((t,i)=>t.bufId>=0?(i+1)+':buf'+t.bufId+(t.mute?' M':'')+(t.loop?' loop':''):null)
            .filter(Boolean).join(' ')
        : 'none recorded')+' · armed:'+(traxArm<0?'none':traxArm+1)
...
const TOUR=[
  ...
  /* Added in R196. The locks had been in for a build and the tour never
     mentioned them, so the only way to find the thing that stops one pad being
     one sound was to press a button marked LOCK and guess. */
  { tab:'seq', el:'btnStepLock', title:'ONE STEP, ITS OWN SOUND',
    body:'<b>LOCK</b>, then tap a step: it plays, and you can give <i>that step alone</i> its own <b>CUTOFF, LEVEL, PAN, DECAY</b> or <b>START</b> — as well as pitch, probability and ratchets.<br><br>Every slider you move plays the step back straight away, so you hear the edit as you make it. This is what stops one pad being one sound repeated sixteen times.' },
  ...
  /* Added in R196, after an audit of what onboarding actually covers: LIVE,
     AMP, DJ, MIX and MIDI appeared in no tour step and no recipe. LIVE is the
     whole other half of the app — nine ways to play, seven voices, a keyboard
     and a tombola — and nothing anywhere told you it was there. Four of the
     ten steps were the sequencer; none was the instrument. */
  ...
  { tab:'pads', el:'btnPlay', title:'THAT IS THE TOUR',
    body:'Press <b>&#9654;</b> to start playing. Your work saves itself as you go, and the <b>PROJ</b> tab keeps named projects, undo history and the bounce/export tools.<br><br>Not covered here but worth finding: <b>MIX</b> is the fader-and-sends view of every pad, <b>AMP</b> takes a guitar or line input, <b>DJ</b> gives you filter and loop performance controls, and <b>MIDI</b> plays hardware or is played by it. Each has its own explainer inside the tab.<br><br>The <b>?</b> button replays this, holds seven step-by-step recipes, and has <b>SCREEN SIZE</b> — worth a look on a smaller phone, where it hands the top bars’ space back to the instrument.' }
];
let tourAt=-1;
...
/* The seventh, against my own note above that six is close to the ceiling. It
   earns the place because the audit that prompted it found LIVE in no tour
   step and no recipe at all: the app's entire instrument half — nine ways to
   play, seven voices, a keyboard — had no route in, and "there's no settings
   to customize the keyboard sound" is what that looks like from outside. It
   ends in a take on a lane, so it still ends in something that exists. */
{ id:'synth', name:'Make the keyboard your own sound',
  blurb:'Pick a voice, shape it until it is yours, then play it over the beat and keep the take.',
  steps:[
  { title:'MAKE THE KEYBOARD YOUR OWN SOUND', body:'The <b>LIVE</b> tab is an instrument, not a preset list. Four steps: choose an engine, shape it, play it, keep it.' },
  { tab:'live', el:'instVoiceSel', title:'CHOOSE AN ENGINE',
    body:'Seven <b>VOICES</b>, and they are seven different methods rather than one with a menu. <b>PHASE SWEEP</b> moves a formant across the note, <b>PULSE</b> moves a notch, <b>CLUSTER</b> stacks six partials that deliberately disagree, <b>FM KEYS</b> is two-operator FM, <b>PLUCK</b> is a modelled string.<br><br>Change it and play the keyboard below — they do not sound alike.',
    waitFor:'pick a VOICE and play a key.',  didIt:'That is the engine.',
    base:()=>S.inst.voice, done:b=>S.inst.voice!==b, tap:'instVoiceSel' },
  { tab:'live', el:'instShapeRow', title:'SHAPE IT',
    body:'<b>SHAPE</b> is that engine’s own main parameter — the line under it says which. On FM KEYS it is the modulation index, on PLUCK the string’s damping, on CLUSTER how far the partials drift apart.<br><br>Move it while you hold a note and you will hear the instrument change, not just get brighter.',
    waitFor:'move SHAPE.', didIt:'That is your sound now.',
    base:()=>instShape(), done:b=>Math.abs(instShape()-b)>0.05, tap:'instShape' },
  { tab:'live', el:'instCut', title:'TONE, AND HOW LONG IT RINGS',
    body:'<b>TONE</b> and <b>RES</b> are one filter across the whole instrument — close it down for something muted, open it and add resonance for a sound with an edge.<br><br><b>ATTACK</b> and <b>RELEASE</b> are multipliers on whatever the voice already does, so the centre is the voice as it comes. Push RELEASE up and a pluck becomes a pad.',
    waitFor:'move TONE or RELEASE.', didIt:'Now it is yours.',
    base:()=>[S.inst.cut,S.inst.rel].join(), done:b=>[S.inst.cut,S.inst.rel].join()!==b, tap:'instCut' },
  { tab:'trax', el:'traxSrc', title:'KEEP WHAT YOU PLAY',
    body:'Set SOURCE to <b>LIVE ONLY</b>, arm a lane with <b>&#9679;</b>, then press PLAY and perform over your beat. The sequencer is backing you can hear but will not capture.<br><br>STOP commits the take, and <b>FX &rarr; TO PAD</b> puts it on a pad so a phrase you played once becomes something you can sequence.',
    waitFor:'set SOURCE to LIVE ONLY.', didIt:'It will capture just your playing.',
    done:()=>$('traxSrc').value==='live' }]},
...
$('btnNewTake').addEventListener('click',()=>{
  S.takeSeed=newTakeSeed(); drawTake(); dirty();
  lcd('NEW TAKE — humanize and probability will play it differently. Bounce again to hear it.');
});
buildDensity();
a11yPass(); a11yWatch();
/* No READY here. offerRestore() decides what happens next — restore a session,
   or build the demo song — and for a new user that build takes a couple of
   seconds and then REPLACES every pad and pattern. Saying "READY · load a
   sample into a pad" before that decision was wrong twice over: it claimed a
   state the app was not in, and it invited work that the arriving song would
   silently throw away. Whichever branch runs now writes its own line when it
   is actually true. */
lcd('STARTING …');
